<div class="card my-3 rounded-0">
  <div class="card-header" role="tab" id="{{ isset($title) ? str_slug($title) :  'permissionHeading' }}">
    <h4 class="mb-0">
      <a role="button" data-toggle="collapse" href="#dd-{{ isset($title) ? str_slug($title) :  'permissionHeading' }}"
      ...
      <div class="row">
        ...
        <div class="col-md-3">
          <div class="pretty p-default">
              ...
              <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" class="form-check-input" {{ $per_found ? 'checked' :''}}>
              <div class="state p-success"><label class="{{ str_contains($perm->name, 'delete') ? 'text-danger' : '' }}">
              {{ $perm->name }}
            </label></div>
          </div>
        </div>
